...
-- 新增：密码保护支持
ALTER TABLE links ADD COLUMN password_type text DEFAULT 'none' CHECK (password_type IN ('none', 'six_digit', 'custom'));
...
-- 为 IP 和时间添加索引以优化查询
CREATE INDEX idx_password_attempts_ip_slug ON password_attempts(ip_address, slug);
CREATE INDEX idx_password_attempts_time ON password_attempts(attempted_at);

-- 允许匿名用户插入和查询（用于验证）
ALTER TABLE public.password_attempts ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public insert to password_attempts"
ON password_attempts FOR INSERT
TO anon, authenticated
WITH CHECK (true);

CREATE POLICY "Allow public select from password_attempts"
ON password_attempts FOR SELECT
TO anon, authenticated
USING (true);
